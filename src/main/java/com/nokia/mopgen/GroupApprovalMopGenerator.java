...
public interface GroupApprovalMopGenerator {
    ...
    static List<MopSection> parseMopFile(String mopContent) {
        ...
        // Pending metadata for the next payload command line
        String pendingDesc     = null;
        String pendingValidate = null;
        ...
        for (String raw : mopContent.split("\n")) {
            ...
            if (trimmed.startsWith("##") && trimmed.length() > 2
                ...
                if (trimmed.startsWith("## Description:")) {
                    ...
                    if (trimmed.startsWith("## cmd-desc:")) {
                        pendingDesc = trimmed.substring("## cmd-desc:".length()).trim();
                    } else if (trimmed.startsWith("## cmd-validate:")) {
                        pendingValidate = trimmed.substring("## cmd-validate:".length()).trim();
                    } else {
                        payloadLines.add(new MopSection.CommandLine(line, pendingDesc, pendingValidate));
                        pendingDesc     = null;
                        pendingValidate = null;
                    }
                }
